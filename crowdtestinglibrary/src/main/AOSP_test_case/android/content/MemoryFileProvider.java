...
public class MemoryFileProvider extends ContentProvider {
    ...
    public boolean onCreate() {
        mOpenHelper = new DatabaseHelper(GlobalRef.applicationContext);
        try {
            OutputStream out = GlobalRef.applicationContext.openFileOutput(DATA_FILE, Context.MODE_PRIVATE);
            out.write(TEST_BLOB);
            out.close();
    ...
    public ParcelFileDescriptor openFile(Uri url, String mode) throws FileNotFoundException {
        ...
        switch (match) {
            ...
            case FILE:
                File file = GlobalRef.applicationContext.getFileStreamPath(DATA_FILE);
